...
#[tokio::main]
pub async fn match_tcp_client(address: String, test_address: String, committee_id:u32, value: Vec<String>, args: Vec<String>, types: String) -> Result<(), Box<dyn Error>> {

    let mut file = OpenOptions::new().append(true).open("output.log").await.unwrap();
    // Connect to a peer    

    let mut individual_check = 0;
    ...
    if types=="individual" && individual_check>=3000
    {
        println!("{:?}", address);
        Ok(())
    }
